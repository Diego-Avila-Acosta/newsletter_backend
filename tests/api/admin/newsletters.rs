...
async fn concurrent_form_submission_is_handled_gracefully() {
    ...
    let response = app.post_send_issue(newletter_request_body.clone());
    let response2 = app.post_send_issue(newletter_request_body);
    let (response, response2) = tokio::join!(response, response2);

    assert_eq!(response.status(), response2.status());
    assert_eq!(
